...
######## Machine Learning #############
######## GAM application  #############
##Forecast Sweden New Cases with Ireland decrease in mobility values##


ireland <- subset(covid19, Country == "Ireland")
ireland <- subset(ireland, days_since_50_cases >= 0)

sweden <- subset(covid19, Country == "Sweden")
sweden <- subset(sweden, days_since_50_cases >= 0)
sweden <- subset(sweden, days_since_50_cases <= 79)

rm(new_df)
new_df <- ireland
new_df <- subset(new_df, days_since_50_cases <= 79)
new_df <- subset(new_df, select = c(9, 17))
...
m_sweden <- gam(New_Cases ~ s(Retail_Recreation,bs="cr",k=3) +
                  s(days_since_50_cases,bs="cr",k=3)
                  ,data = sweden)
corr <- cor.test(x=sweden$New_Cases, y=sweden$Retail_Recreation, method = 'spearman')
summary(m_sweden)

#prediction with Irelands decrease in retail values
pred_sweden <- predict(m_sweden, newdata = new_df,
                       interval = "confidence")
sum_pred <- round(sum(pred_sweden), digits = 0)
sum_pred
